---
title: "Cartridge Runtime"
layout: doc
permalink: /13.1-cartridge-runtime/
---
# Cartridge Runtime
...
The manifest must always include `identity_cap()` — the identity capability required for handshake verification (see [12.3-HANDSHAKE.md](/docs/12.3-handshake)). `CartridgeRuntime::new()` panics if `CAP_IDENTITY` (`cap:effect=none`) is missing.
...
When a REQ arrives, the runtime finds the handler by matching the request's cap URN against registered cap URNs using `is_dispatchable(candidate, request)` (see [../07-DISPATCH.md](/docs/07-dispatch)). When multiple handlers match, the runtime ranks by specificity — preferring exact matches, then refinements (more specific than requested), then fallbacks (more generic).
...
flowchart TD
    ...
    D -->|slug match| G["CLI Mode<br/>Parse args from cap defs<br/>Run handler, output RAW to stdout"]
...
- **Any arguments**: CLI mode. The runtime parses arguments from cap definitions and runs the matching handler directly. Output goes to stdout.
...
3. **Read loop**: Read frames from stdin in a loop.
   - **REQ**: Look up a handler by `cap_urn`, resolving the registered pattern that will serve it. Admit through the cap's whole pool chain (see Concurrency Pools below); if any chain pool is full, enqueue on the cap's singleton queue and send a `level="queued"` LOG naming the queue position and blocking pool. Admitted requests spawn a new handler task (`tokio::spawn`) with its own `InputPackage` and `OutputStream`. Record the request ID so continuation frames can be routed to this task.
   - **Heartbeat**: Apply any `desired_capacities` the probe carries (an unknown pool refuses the whole batch with an `UNKNOWN_POOL` ERR in place of the reply), then respond with a Heartbeat carrying the same ID and the mandatory refreshed `pools` state map. This is handled inline — no task spawned.
...
`run_with_keepalive()` solves this by moving blocking work to `tokio::task::spawn_blocking` (a separate thread pool), freeing the tokio workers so the writer task can flush keepalive frames. See [13.4-PROGRESS-AND-LOGGING.md](/docs/13.4-progress-and-logging).

## Concurrency Pools

The runtime's one capacity concept (`bifaci::pools`). A cap IS a pool of one:
the runtime materializes one singleton pool per registered handler pattern
(keyed by the canonical cap URN), every shared pool the manifest declares
(`PoolDeclarations` — memberships must resolve to registered patterns, a
mismatch is a hard startup error), and the reserved `all` pool containing
every cap. The retired scalar `handler_capacity` does not exist; `all`'s
declared capacity is its exact replacement.

Each pool carries three numbers: `declared` (the manifest's shipped default),
`configured` (the operator's number, delivered via heartbeat probes and
starting equal to `declared`), and optional `available` (the cartridge's
self-report — absent means static, the normal case). The effective admission
bound is `min(configured, available)` with `0` meaning unlimited everywhere.

A dispatch admits through its cap's whole CHAIN — singleton, declared pools
containing it, `all` — atomically: every pool must have room or the request
queues on its singleton queue (queues lead to pools; shared pools have no
queue of their own). Completion releases the whole chain, then admits the
oldest eligible ticket across all singleton queues — arrival-ordered by a
global ticket, never cap-biased. `pool_handle(name)` gives a handler a
handle that writes `available` only (e.g. lowering `gpu` to 0-concurrency
during a model load); it never touches `declared` or `configured`.

The full pool-state map rides the HELLO meta and every heartbeat reply; the
host folds it into roster stats, so the engine's admission and both clients
see the same numbers the runtime enforces.
...
- **`peer()`** → `&dyn PeerInvoker`: For calling other cartridges' capabilities. See [13.3-PEER-INVOCATION.md](/docs/13.3-peer-invocation).
...
let data = require_stream(&streams, "media:ext=pdf")?;

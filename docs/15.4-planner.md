...
This is different from the orchestrator (see [15.1-ORCHESTRATOR.md](/docs/15.1-orchestrator)), which takes an explicit DAG description. The planner builds the DAG by searching the capability graph.
...
```mermaid
graph LR
    PDF["media:ext=pdf"] -->|extract_metadata| REC["media:record"]
    PDF -->|generate_thumbnail| IMG["media:ext=png;image"]
    PDF -->|extract_text| TXT["media:enc=utf-8"]
    TXT -->|generate_embeddings| EMB["media:embedding"]
    TXT -->|summarize| TXT2["media:enc=utf-8"]
...
- `Single`: One value (e.g., `media:ext=pdf` — one PDF document).
- `Sequence`: Multiple values (e.g., `media:ext=pdf;list` — a list of PDF documents). The `list` marker tag in the media URN signals this.
...
```mermaid
flowchart TD
    Q{"slot_values<br/>[token_id:slot_name]?"} -->|Found| V1["Use explicit<br/>per-step value"]
...
1. **`slot_values["{token_id}:{slot_name}"]`** — Explicit per-step values, addressed by the step's `StepToken` (the `StrandStep::token_id` the planner minted, which is also the plan's cap-node id). A token comes from the plan or it does not exist: it is never derived from a position, because a strand is a DAG — parallel branches merging downstream have no ordinal, and two identical caps on separate branches differ only by token.
...
## Live Feeds Need No Planner Support

A live-feed source (13.2 §Reference Media) requires nothing from the
planner, deliberately. The consuming arg's `is_sequence` declaration
already makes every wiring decision: a sequence-consuming cap (`concat`)
links directly to the sequence producer and receives the stream whole —
over an unbounded feed this is a valid **deferred-emission** plan, since
stopping the run delivers END and the fold emits (a `concat` over a
microphone is "the whole recording, when stopped"); an item-consuming cap
(a per-frame filter) gets a ForEach inserted, exactly as for any bounded
sequence. Emission timing is op policy, not type structure — there is no
plan-time "unbounded" restriction, and unboundedness does not appear in
the plan at all. Memory for completion-requiring ops over long feeds is an
operational concern of the op (buffer, spill, window), surfaced through
ordinary memory-pressure machinery, never a planner error.

...
layout: doc
permalink: /13.4-progress-and-logging/
---
...
Every LOG frame carries non-empty `level` and `message` text in metadata key 5.
Non-progress LOG frames also carry a mandatory `attribution_class` (`input`,
`resource`, `environment`, or `internal`) and may carry one non-empty
source-declared `arg_urn`. Progress LOG frames instead carry a `progress` float
and never carry diagnostic attribution.

LOG frames do not affect the data stream. Progress drives live progress state.
Non-progress diagnostics are forwarded live with the stable execution step
token and optional body ordinal; clients retain a bounded in-memory window.
They are not persisted as structured run history. Receivers never infer a
missing class or argument attribution, and malformed attribution is a protocol
error.
...
| `"queued"` | Emitted by the runtime (not handlers) when a REQ waits for pool capacity: names the request's 1-based queue position and the pool it is queued on. The pipeline's stall timer keeps pausing on it. |
...
req.output().log(
    ...
    "Found 12 images in document",
);

// Diagnostic attributed by its source to one cap argument
req.output().log_for_argument(
    "warn",
...
## Final Progress Rides in END

The request's final progress is **terminal metadata on the END frame** (L3/L5),
not a LOG frame. When a handler completes successfully, the runtime stamps
`progress` (default 1.0) and an optional `message` into the END's meta; a
handler can override both via `OutputStream::finish(progress, message)` before
returning. The receiving side delivers this value as the final progress event
— because it travels IN the terminal frame, it cannot race it.

Consequences:

- **Do not emit a trailing 100% progress LOG.** END is the single authoritative
  terminal progress event, so completion cannot race a separate LOG frame.
- END/ERR is the LAST flow frame of a request (L4). The cartridge writer
  thread enforces this: a flow frame reaching it after the flow's terminal has
  been written (a detached `ProgressSender`, a keepalive ticker that outlived
  the handler) is dropped and counted (`post_terminal`), never written. The
  same reason covers the RECEIVER side of the law: a frame that crossed its
  request's terminal on the wire (a credit grant, a straggling LOG) is
  classified `post_terminal` by every routing runtime — see
  [12.5 — Flow Control](/docs/12.5-flow-control) §"The teardown race" —
  keeping `no_route` a genuine routing-anomaly alarm. Drops are observable in
  the runtime's protocol stats — nothing is silent.
- A successful END without explicit terminal progress reads as 1.0. A failed
  END never synthesizes a completion value.
...
See [15.3-PROGRESS-MAPPING.md](/docs/15.3-progress-mapping) for how `ProgressMapper` deterministically subdivides progress ranges across steps.
...
The only way to emit observable output from a cartridge is through LOG frames
via `output.log()`, `output.log_for_argument()`, and `output.progress()`. These
send frames through the stdout channel, which the host receives and processes.

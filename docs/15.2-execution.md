...
no two adjacent caps' pool chains share a bounded pool. Within such a segment,
...
shared-bounded-pool adjacencies use the materializing path.

A shared bounded pool between two adjacent caps is an execution boundary
because an admission permit represents ownership of concrete pool slots from
REQ through terminal response. The executor runs the first cap to completion,
materializes its output, and only then acquires the dependent invocation. It
must not pre-acquire both: adjacent caps whose chains meet in a capacity-one
pool would leave the first invocation unfed while acquisition of the second
waits for the slot the first holds. Two same-cartridge caps in DISJOINT
bounded pools share no permit and keep streaming — the pool refinement over
the old whole-cartridge domain. This boundary preserves FIFO admission and
process-loss isolation; it is not a retry or fallback.
